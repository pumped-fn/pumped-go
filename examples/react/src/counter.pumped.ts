...
const configController = provide((scope) => {
	return {
		changeIncrement: (increment: number) =>
			scope.update(config, (config) => ({ ...config, increment })),
		changeInterval: (interval: number) =>
			scope.update(config, (config) => ({ ...config, interval })),
	};
});

const counter = provide(() => mutable(0));

const timer = derive([config], ([config], scope) => {
	const interval = setInterval(() => {
		scope.update(counter, (v) => v + config.increment);
	}, config.interval);
