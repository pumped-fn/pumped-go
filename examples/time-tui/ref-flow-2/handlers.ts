...
export const validateOrder = validateOrderFlow.handler(
  ...
  async ({ db, shipping, logger }, ctx, input) => {
    logger.info("Validating order", { customerId: input.customerId });

    const customerResult = await ctx.execute(
      async (customerId: string) => db.findCustomer(customerId),
      input.customerId,
      (error) => ({
        code: "INVALID_CUSTOMER" as const,
        message: error instanceof Error ? error.message : "Customer lookup failed",
        details: error,
      })
    );
    ...
    if (customerResult.type === "ko") {
      ...
      return ctx.ko(customerResult.data);
    }
    ...
    if (!addressResult.data.valid) {
      return ctx.ko({
        code: "INVALID_ADDRESS",
        message: addressResult.data.error || "Invalid address",
      });
    }
    ...
    for (const item of input.items) {
      ...
      if (inventoryResult.type === "ko") {
        ...
        return ctx.ko({
          ...
          message: `Failed to check inventory for ${item.productId}`,
...
export const processPayment = processPaymentFlow.handler(
  ...
  async ({ payment, logger }, ctx, input) => {
    logger.info("Processing payment", { amount: input.amount });
...
export const updateInventory = updateInventoryFlow.handler(
  ...
  async ({ db, logger }, ctx, input) => {
    logger.info("Updating inventory", { orderId: input.orderId });
...
export const processOrder = processOrderFlow.handler(
  { db, shipping, logger },
  async ({ db, shipping, logger }, ctx, input) => {
    logger.info("Processing order", { customerId: input.customerId });
    ...
    const trackingNumberData = shippingResult[0].data;
    const shippingInfo = shippingResult[1].data;
    const estimatedDelivery = new Date();

    // Handle different possible return types from shipping services
    const trackingNumber = typeof trackingNumberData === 'string' ? trackingNumberData : 'TRK123';
