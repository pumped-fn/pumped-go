...
(async () => {
  ...
  const inventoryResult = await flow.execute(processOrder, inventoryFailOrder, {
    scope,
  });
  ...
  const paymentResult = await flow.execute(processOrder, paymentFailOrder, {
    scope,
  });
  ...
  const addressResult = await flow.execute(processOrder, addressFailOrder, {
    scope,
  });
