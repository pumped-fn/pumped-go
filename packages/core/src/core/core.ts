...
export type InferOutput<T> = T extends Promise<infer X>
	? InferOutput<X>
...
export const isResouceOutput = <T>(
	value: unknown,
): value is ResourceOutput<T> =>
...
function isExecutor<T>(value: unknown): value is Executor<T> {
	return (
		...
		(value as Executor<T>)[executorSymbol]
	);
}
...
export interface Executor<T> {
	...
	dependencies:
		| Executor<unknown>[]
		| Record<string, Executor<unknown>>
		| undefined;
}
...
export async function resolve<T>(
	...
	const entries = await Promise.all(
		Object.entries(input).map(async ([key, executor]) => [
			key,
...
export const derive = <T extends Array<unknown> | object, R>(
	...
	factory: (
		dependency: { [K in keyof T]: InferOutput<T[K]> },
		scope: Scope,
	) => R | Promise<R>,
): Executor<R> => {
	return {
...
class BaseScope implements Scope, ScopeInner {
	...
	async #resolveDependencyArray(
		dependencies: Executor<unknown>[],
	): Promise<unknown[]> {
	...
	async #evalute<T>(executor: Executor<T>): Promise<GetAccessor<Awaited<T>>> {
		this.#ensureNotDisposed();
		const container =
			this.#values.get(executor) ||
			({ kind: "pending", promise: null as unknown } as Container);

		const willResolve = new Promise((resolve, reject) => {
