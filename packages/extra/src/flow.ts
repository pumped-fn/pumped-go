...
export declare namespace Flow {
  ...
  export type AsyncUnit<Input, Output> = (
    input: Input,
    context: FlowController
  ) => Promise<Output>;
  ...
  export type FlowController = {
    ...
    executeWithTimeout: <Input, Output>(
      flow: FlowUnit<Input, Output>,
      input: Input,
      timeoutMs: number,
      option?: ExecutionOption
    ) => Promise<Output>;
    executeWithRetry: <Input, Output>(
      flow: FlowUnit<Input, Output>,
      input: Input,
      retryOptions: RetryOptions,
      option?: ExecutionOption
    ) => Promise<Output>;
    executeMany: <Input, Output>(
      flow: FlowUnit<Input, Output>,
      inputs: Input[],
      option?: ExecutionOption & { concurrency?: number }
    ) => Promise<Output[]>;
...
export const flowScoped = meta("pumped-fn/flow-scoped", custom<boolean>());
...
function createFlow<I, O>(
  ...
  flow: Core.Executor<Flow.AsyncUnit<I, O>> | Flow.AsyncUnit<I, O>
): Flow.Flow<I, O> {
...
async function execute<Input, Output>(
  ...
  const executionScope = scope.pod();
  ...
  const controller: Flow.FlowController = {
    ...
    safeExecute: async function <SubInput, SubOutput>(
      ...
      // Parse retry options
      const retryOptions: Flow.RetryOptions =
        typeof option?.retry === "number"
      ...
      try {
        ...
        executionRecord.metrics.duration =
          executionRecord.metrics.endTime - executionRecord.metrics.startTime;
        executionRecord.metrics.status =
          ...
          return Promise.race([
            ...
            sleep(option.timeout).then(() => {
              throw new TimeoutError(
                `Flow execution timed out after ${option.timeout}ms`
              );
            }),
          ]);
        }
    ...
    executeWithRetry: async function <SubInput, SubOutput>(
      ...
      return this.execute(subFlow, subInput, {
        ...option,
        retry: retryOptions,
      });
    },
    ...
    executeMany: async function <SubInput, SubOutput>(
      ...
      if (errors.length > 0) {
        throw new Error(
          `Failed to execute ${errors.length} of ${inputs.length} flows`
        );
      }
  ...
  return await executionScope
    ...
    .finally(async () => {
      scope.disposePod(executionScope);
    });
}
