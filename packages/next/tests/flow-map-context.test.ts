import { flow, provide, dataAccessor, custom } from "../src";
...
describe("flow map context and opts plugins", () => {
  test("data accessors provide type-safe context access", async () => {
    ...
    const countAccessor = dataAccessor('count', custom<number>());
    const messageAccessor = dataAccessor('message', custom<string>());
  ...
  test("opts.plugins are added to nested execution", async () => {
    ...
    const parentFlow = flow.derive(
      ...
      async ({ childFlow }, input, controller) => {
        ...
        await controller.execute(childFlow, {}, {
  ...
  test("Symbol keys prevent collision between plugins", async () => {
    ...
    const plugin1Accessor = dataAccessor<string>('shared', custom<string>());
    const plugin2Accessor = dataAccessor<number>('shared', custom<number>());
    ...
    const { result } = await flow.execute(testFlow, {}, {
