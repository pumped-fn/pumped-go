...
// Array dependencies
const app = derive([config, database, logger], (cfg, db, log) =>
  createApplication(cfg, db, log)
);
...
const configurableService = derive([config.reactive, logger], (cfg, log) =>
  createConfigurableService(cfg, log)
);
...
const createCacheService = (
  initialState: CacheState = { entries: new Map() }
) => ({
...
const apiService = derive([apiConfig.reactive, httpClient], createAPIService);
...
### Using flow to organize workload

`@pumped-fn/core-next` exposes `flow` to organize flow concept

The flow can be created as easy as

```typescript
import { flow, custom } from "@pumped-fn/core-next";

const login = flow.provide(
  {
    name: "login", // optional,
    input: custom<string>, // required - StandardSchema compatible, can use any other libraries
    output: custom<string>, // required - StandardSchema compatible, can use any other libraries
  },
  async (
    input, // verified by standardSchema, type-guarded by `input`
    controller // { execute(resoledFlow), { input ...}}
  ) => {
    // any logic
    return ""; // because the output is set to be string, type guarded
  }
);
```

The flow can be derived from other subflows or any other executors

```typescript
const auditUserLogin = flow.derive(
  {
    dependencies: [betterLogExecutor],
    input: custom<{ username: string, attempts: number, ip: string, ...}>(),
    output: custom<void>()
  },
  async ([beterLogResolved], input) => {
    // log to better log
  }
)

const login = flow.derive(
  {
    name: "login", // optional,
    dependencies: [auditUserLogin, authGateway],
    input: custom<string>, // required - StandardSchema compatible, can use any other libraries
    output: custom<string>, // required - StandardSchema compatible, can use any other libraries
  },
  async (
    [auditUserLogin, authGateway],
    input, // verified by standardSchema, type-guarded by `input`
    controller // { execute(resoledFlow, param), safeExecute(flow, param) { input ...}}
  ) => {
    await controller.safeExecute(
      auditUserLogin, {
        username: ..., attemps ... // type guarded
      })
